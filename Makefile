...
APPNAME=mtb-example-pmg1-dfu-bootloader
...
POSTBUILD=$(CY_TOOL_cymcuelftool_EXE_ABS) --sign $(MTB_TOOLS__OUTPUT_CONFIG_DIR)/$(APPNAME).elf --output $(MTB_TOOLS__OUTPUT_CONFIG_DIR)/$(APPNAME).elf --hex $(MTB_TOOLS__OUTPUT_CONFIG_DIR)/$(APPNAME).hex

################################################################################
# Include additional configurations, source and headers.
################################################################################
DFU_PATH=$(SEARCH_dfu)/config/CAT2

ifeq ($(TRANSPORT_OPT), I2C)
    $(info Selected I2C transport for DFU.)
else ifeq ($(TRANSPORT_OPT), USB_CDC)
    $(info Selected USB_CDC transport for DFU.) 
...
SOURCES+=$(DFU_PATH)/transport_$(TRANSPORT_OPT_LOWER_CASE).c
INCLUDES+=$(DFU_PATH)
